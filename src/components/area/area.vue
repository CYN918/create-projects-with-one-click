				...
				<div class="region-wrapper" v-for="(province,index) in data" :key="index">
						<Poptip placement="right-start" width="250" trigger="hover">
								<div class="provice-wrapper">
										<Checkbox :value="province.checkAll" @click.prevent.native="handleCheckProvince(province)">
												{{province.label}}
										</Checkbox>
								</div>
								<div class="city-wrapper" slot="content">
										<Checkbox class="city-item" v-for="(city,childIndex) in province.children" :key="childIndex"
															:value="city.isCheck" @click.prevent.native="handleCheckCity(province,city)">
												{{city.label}}
										</Checkbox>
								</div>
						</Poptip>
