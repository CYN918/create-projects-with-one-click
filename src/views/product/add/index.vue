<template>
		<div class="margin">
						...
						<div class="mbottom">
								<div class="goods-fitter">
										<div class="search-wrapper1">
												<el-page-header @back="goback" :content="title_text"></el-page-header>
				...
				<div class="margin" style="margin-right:0">
						<div class="msetting-account-wrapper" style="margin-left: -20px;">
								...
								<div class="tabs-content">
										<account-general v-show="currentIndex===0" :valueObj='brandObj' :res="data"
																		 ref="genaral"></account-general>
				...
				<div style="margin-top: 20px; text-align: center;">
						<el-button type="danger" size="small" @click="resert">重置</el-button>
...
<script>
    ...
    import { throttleTipPop } from '@/utils/throttle' //节流弹窗
    ...
    export default {
        ...
        methods: {
            ...
            _addProduct() {
                ...
                console.log(333, data.genaralForm)
                data.genaralForm.first_image_url.forEach(item => {
                    item.imgUrl = item.url
                    item.imgDesc = item.imgDesc
                })
                ...
                table.forEach(item => {
                    skus.push({
                        ...
                        volumePrices: [
                            {
                                volumeNumber: item.volumeNumber1,
                                volumeCost: item.volumeCost1,
                                volumeCarriage: item.volumeCarriage1,
                            },
                            {
                                volumeNumber: item.volumeNumber2,
                                volumeCost: item.volumeCost2,
                                volumeCarriage: item.volumeCarriage2
                            },
                            {
                                volumeNumber: item.volumeNumber3,
                                volumeCost: item.volumeCost3,
                                volumeCarriage: item.volumeCarriage3
                            },
                            {
                                volumeNumber: item.volumeNumber4,
                                volumeCost: item.volumeCost4,
                                volumeCarriage: item.volumeCarriage4
                            },
                        ], //集采批发价设置
                    })
                })
                data.genaralForm.productSize = data.genaralForm.productSizelong + ',' + data.genaralForm.productSizewide + ',' + data.genaralForm.productSizeheight
                data.genaralForm.grossWeight = data.genaralForm.grossWeight
                data.genaralForm.packSize = data.genaralForm.packSizelong + ',' + data.genaralForm.packSizewide + ',' + data.genaralForm.packSizeheight
                data.genaralForm.packNumber = data.genaralForm.packNumber
                if(data.genaralForm.tableData.length){
                    if (this.addGoodsForm.id) {
                    //判断是否是虚拟
                    if (this.isReal == 1) {
                        console.log(444, this.isReal)
                        editorProduct(data, skus, 1, this.addGoodsForm.id, goodsAttributes).then(res => {
                            if (res.code == 200) {
                                throttleTipPop(this,'success','修改商品成功')
                                this.$router.go(-1)
                            }
                        })
                    } else {
                        editorProduct(data, skus, 0, this.addGoodsForm.id, goodsAttributes).then(res => {
                            if (res.code == 200) {
                                throttleTipPop(this,'success','修改商品成功')
                                this.$router.go(-1)
                            }
                    ...
                    if (this.isReal == 1) {
                        addProduct(data, skus, 1, goodsAttributes).then(res => {
                            if (res.code == 200) {
                                throttleTipPop(this,'success','商品添加成功')
                                this.$router.go(-1)
                            }
                        })
                    } else {
                        addProduct(data, skus, 0, goodsAttributes).then(res => {
                            if (res.code == 200) {
                                throttleTipPop(this,'success','商品添加成功')
                                this.$router.go(-1)
                            }
                    ...
                    throttleTipPop(this,'error','请添加对应的商品规格')
                }
            }
        },
        created() {
            if (this.$route.query.isReal){
