						...
						<div class="goods-fitter">
								<el-form ref="form" :model="form" label-width="100px" size="small">
										<div class="search-wrapper">
												...
												<el-button type="primary" size="small" style="margin-left: 60px;margin-right: 10px" @click="cancel">取消分类
												</el-button>
												<el-button type="primary" size="small" @click="start">启动分类</el-button>
												<br/>
												<div style="width: 100%;display: inline-block;">
														<div class="row_css">
																...
																<el-radio-group v-model="radio1" style="display: flex;flex-wrap: wrap">
																		<div v-for="(item, index) in proClassication1" style="margin-left: 10px;display: flex;align-items: center;width: 168px">
																				<el-checkbox v-model="item.multi_select1" style="margin-right: 7px;vertical-align: middle;"
												...
												<el-row :gutter="10">
														<el-col :span="24" style="text-align: center;margin-top: 20px;margin-bottom: 20px">
																<el-button type="primary" @click="addSubmit"
																					 ...
																					 element-loading-text="添加中,请稍后~">添加到我的苏宁
																</el-button>
														</el-col>
												</el-row>
												<!--分页-->
...
<script>
    import {findCategoryList, getBuyer, getDatalist2, importChooseGoods2, startcancel} from "@/api/product/index";
    ...
    export default {
        data() {
            return {
                ...
                proClassication2: [],//一级分类数据源
                proClassication3: [],//一级分类数据源
        ...
        methods: {
            ...
            changestate(event, item) {
                ...
                if (event) {
                    this.newList.push({catId: item.catId, catClass: item.catClass, source: 3, enable: item.enable})
                } else {
            ...
            change_one(item, index) {
                ...
                console.log(999, this.form.catId)
                // 获取列表数据
                this.getDataList(this.form)
            },
            //获取分类数据
            ...
            getDataList(val) {
                ...
                if (val) {
                    // console.log(111, val.catClass)
                    let obj = {}
                    obj = JSON.parse(JSON.stringify(this.form))
                    // obj.pageIndex = this.form.pageIndex
                    // obj.pageSize = this.form.pageSize
                    // obj.catIdOne = val.catId
                    // obj.catIdTwo = val.catId
                    console.log(789, val.categoryId)
                    // obj.catId = val.categoryId
                    // console.log(obj)

                    getDatalist2(obj).then(res => {
                        if (res.code == 200) {
                            let {data} = res
            ...
            importOne(item, index) {
                ...
                let newArr = []
                newArr.push(obj)
                importChooseGoods2(newArr).then(res => {
            ...
            addSubmit() {
                ...
                if (this.checkList.length > 0) {
                    importChooseGoods2(this.checkList).then(res => {
                        let {code} = res
                        if (code == 200) {
                    ...
                    // let newArr = []
                    // newArr.push(obj)
                    importChooseGoods2(checkList2).then(res => {
                        let {code} = res
                        if (code == 200) {
                            this.$message.success("添加成功")
                            this.dataList.forEach(item2 => {
