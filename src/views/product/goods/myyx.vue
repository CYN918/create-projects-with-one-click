												...
												<div style="width: 100%;display: inline-block;">
																...
																<el-radio-group v-model="radio1" style="display: flex;flex-wrap: wrap">
																		<div v-for="(item, index) in oneList"
																				...
																				<el-checkbox v-model="item.multi_select1"
																										 ...
																										 style="margin-right: 7px;vertical-align: middle;"></el-checkbox>
														...
														<div class="row_css">
																...
																<el-radio-group v-model="radio2" style="display: flex;flex-wrap: wrap">
																		<div v-for="(item, index) in twoList" style="margin-left: 10px;display: flex;align-items: center;width: 168px">
												...
												<el-row :gutter="10">
														<el-col :span="24" style="text-align: center;margin-top: 20px;margin-bottom: 20px">
																<el-button type="primary" @click="addSubmit"
																					 ...
																					 element-loading-text="添加中,请稍后~">添加到我的严选
																</el-button>
														</el-col>
												</el-row>
												<!--分页-->
...
<script>
    import {findCategoryList, getBuyer, getDatalist3, importChooseGoods3, startcancel} from "@/api/product/index";
    ...
    export default {
        ...
        methods: {
            ...
            changestate(event, item) {
                ...
                if (event) {
                    this.newList.push({catId: item.catId, catClass: item.catClass, source: 2, enable: item.enable})
                } else {
            ...
            change_one(item, index) {
                ...
                this.radio2 = ''
                //获取二级分类
                // this.oneList.forEach(item2 => {
                //     if (item2.id == item.id) {
                //         this.twoList = item2.subCateList
                //         this.form.catIdOne = item2.id
                //     }
                // })
                // this.twoList
                let obj = {}
            ...
            getList() {
                ...
                findCategoryList(obj).then(res => {
                    if (res.code == 200) {
                        ...
                        //     this.List_one.push({"id": item.id, "name": item.name});
                        // });
                        this.oneList  = this.oneList.map(v => {
            ...
            getDataList(item) {
                ...
                if (item) {
                    console.log(111, item.catClass)
                    let obj = {}
                    obj = JSON.parse(JSON.stringify(this.form))

                    getDatalist3(obj).then(res => {
                        if (res.code == 200) {
                            let {data} = res
            ...
            importOne(item, index) {
                ...
                let newArr = []
                newArr.push(obj)
                importChooseGoods3(newArr).then(res => {
            ...
            addSubmit() {
                ...
                if (this.checkList.length > 0) {
                    importChooseGoods3(this.checkList).then(res => {
                        let {code} = res
                        if (code == 200) {
                    ...
                    importChooseGoods3(checkList2).then(res => {
                        let {code} = res
                        if (code == 200) {
                            this.$message.success("添加成功")
                            this.getList()
