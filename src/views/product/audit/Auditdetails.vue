<template>
  <div class="margin">
    <el-form ref="form" label-width="260px" size="small">
      ...
      <div class="tabs-content" v-for="(item,index) in data" :key="index">
        <div class>
          <div class="mbottom">
            <div class="goods-fitter">
              <el-form label-width="260px" size="small">
                <div class="search-wrapper">
                  <h4 class="title">
                    ...
                    <span>审核状态：{{item.auditStatus == 0?'未审核 ':item.auditStatus == 1?'审核通过':'审核不通过'}}</span>
                    <span>审核发起人：{{item.createUserName}}</span>
                    <span>审核发起时间：{{item.createTime}}</span>
                    <span>审核模式：{{item.auditMode  | modeName}}</span>
                  </h4>
                  <br />
                  <div class="wrapper">
                    <ul class="list">
                      <li class="item" v-for="(data,index2) in item.processList" :key="index2">
                        ...
                        <div class="word_line">
                          ...
                          <span class="t1">{{data.auditUserName}}
                             <span v-if="data.organizeName && data.roleName"> ({{data.roleName}} | {{data.organizeName}} )</span>
                             <span v-if="data.organizeName && data.roleName === null"> ({{data.organizeName}})</span>
                             <span v-if="data.roleName && data.organizeName === null"> ({{data.roleName}})</span>
                          </span>
                        </div>
                        <div class="word_line">
                        ...
                        <div class="word_line">
                          ...
                          <span class="t1">{{data.auditPower === 4?'决策 ':'查阅'}}</span>
                        </div>
                        <div class="word_line">
...
<script>
import { getAllaudit } from "@/api/product/index";
...
export default {
  ...
  filters: {
    ...
    modeName(val){
      const arr = [
        { auditType: 1, name: "顺序审核" },
        { auditType: 2, name: "全部审核" },
        { auditType: 3, name: "一票审核" },
      ];
      for (var i = 0; i < arr.length; i++) {
        if (val == arr[i].auditType) {
          return arr[i].name;
