        ...
        <el-table :data="tableData" border stripe @row-click="aftergoodslistb">
          ...
          <el-table-column label="客诉人信息" align="center" width="200">
            <template slot-scope="scope">
              ...
              <div
                class="txt_left"
              >物流公司：{{scope.row.returnShipping?scope.row.returnShipping:'暂无信息'}}</div>
              <div
                class="txt_left"
              >物流单号：{{scope.row.returnLogisticNo?scope.row.returnLogisticNo:"暂无信息"}}</div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="升级客诉" align="center" width="150">
            <template slot-scope="scope">
              <div class="why_div">
                <div v-for="(item,index) in scope.row.afterUpgradeComplains" :key="index">
                  <div class="txt_left color_red">{{index+1}}次升级</div>
                  <div class="txt_left color_red">客诉原因：{{item.upgradeReason}}</div>
                  <div class="txt_left">
                    <el-button type="primary" size="small" @click="attachment(item,true)">查看附件</el-button>
                  </div>
                </div>
              </div>
            </template>
          ...
          <el-table-column label="处理方法" align="center">
          ...
          <el-table-column label="进度说明" align="center" width="130">
            <template slot-scope="scope">
              ...
              <div class="txt_left">申请原因：{{scope.row.afterReason}}</div>
              <div class="txt_left">
            ...
            <template slot-scope="scope">
              <div :key="scope.row.afterId">
                ...
                <div
                  class="small-Idiv"
                  @click.stop="refundfunc(scope.row)"
                  v-if="scope.row.returnType== 6 &&　scope.row.canHandle　== 1 &&　(scope.row.logisticsNo == '' || scope.row.logisticsNo == null)"
          ...
          <el-table-column label="操作" align="center" width="150">
            <template slot-scope="scope">
              <div class="line-div small_top" style="padding-bottom: 0">
                <div
                  class="small-Idiv"
                  @click.stop="productfunc(scope.row)"
...
export default {
  ...
  methods: {
    ...
    async attachment(val, type) {
      if (type) {
        var { data } = await findpicb(val.upgradeId);
      } else {
        var { data } = await findpicb(val.complainId);
      }
      this.imgArr = data || [];
