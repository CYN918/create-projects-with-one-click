<template>
  <div class="aduit_dilog">
    <!-- 审核 -->
    <el-dialog title="财务审核" :visible.sync="dialogVisible" width="820px" :before-close="handleClose">
      <div>
        <el-form
          ref="ruleForm"
          label-width="100px"
                ...
                <div class="goods-fitter">
                  <div class="search-wrapper1">
                    ...
                    <div class="right_btn">
                      ...
                      <el-button type="primary" size="small"  @click="details">查看详情</el-button>
                    </div>
                  </div>
                </div>
              </div>
...
import { checkfinanceb, findfinanceauditdetail } from "@/api/order";
...
export default {
  ...
  methods: {
    details(){
      this.$parent.aftersales(this.storedata,true)
    },
      ...
      this.$refs.ruleForm.validate(async valid => {
        if (valid) {
          const obj = {
            ...
            organizeId: this.ruleForm.organizeName,
            sysUserId: this.sysobj.sysuserId,
            sysUserName: this.sysobj.sysuserName
          };
          const data = await checkfinanceb(obj);
          if (data.code == 200) {
            this.$message({
  ...
  watch: {
    ...
    async storedata(val) {
      const { data } = await findfinanceauditdetail(val.complainId);
      this.ruleForm = data;
      this.ruleForm.organizeName = data.roles[0].organizeId
      this.ruleForm.roleName = data.roles[0].roleName
      this.options = []
      this.getchildnode();
    }
  }
