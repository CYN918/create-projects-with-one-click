																				...
																				<el-table-column prop="freightAddress" label="运送到" align="center" width="600">
																						<template slot-scope="scope">
																								...
																								<el-popover trigger="click" placement="bottom" width="840">
																										<el-checkbox v-for="(province,index) in scope.row.dataList"
																										...
																										<div slot="reference" class="name-wrapper">
																												<span class="idNick_cs">{{scope.row.id}}</span>
    ...
    export default {
        ...
        methods: {
            ...
            submitData(data) {
                ...
                if (this.showItem1) {
                    ...
                    table1 = obj.freightCalculate['1'].map((item) => {
                        ...
                        data1.freightAddress = item.dataList.map((item2) => {
                            const data2 = {
                ...
                if (this.showItem2) {
                    ...
                    for (var key in table2) {
                        ...
                        // console.log(789,table2[key].freightAddress)
                ...
                this.freightAddress.forEach(item => {
            ...
            getArea() {
                getAllArea().then(res => {
                    if (res.code == 200) {
                        ...
                        //处理value和label
                        this.freightAddress = JSON.parse(JSON.stringify(this.freightAddress).replace(/id/g, 'addressId'))    //data为数组，title为修改前，name为修
                        this.freightAddress = JSON.parse(JSON.stringify(this.freightAddress).replace(/regionName/g, 'addressName'))
                        this.freightAddress = JSON.parse(JSON.stringify(this.freightAddress).replace(/childRegion/g, 'childAddress'))
                        this.freightAddress.map(v => {
            ...
            getList() {
                for (var i in this.list.freightCalculate) {
                    this.list.freightCalculate[i] = this.list.freightCalculate[i].map(v => {
                        this.$set(v, 'origin_list', this.freightAddress)//----每行源数据
                        this.$set(v, 'dataList', [])
                        this.$set(v, 'id', '')
            ...
            handleCheckProvince(param, row, index) {
                ...
                if (!param.isCheck) {
                    for (let i in param.childAddress) {
                        ...
                        if (row.id) {
                            if (row.id.indexOf(param.childAddress[i]) == -1) {
                                row.id = row.id.replace(param.childAddress[i].addressName, ' ')
                            }
                            row.id = row.id + ' ' + param.childAddress[i].addressName
                        } else {
                            row.id = param.childAddress[i].addressName
                        }
                    }
                } else {
                    //全不选
                    for (let i in param.childAddress) {
                        ...
                        if (row.id.indexOf(param.childAddress[i].addressName) !== -1) {
                            row.id = row.id.replace(param.childAddress[i].addressName, ' ')
                        }
                    }
                }
            ...
            handleCheckCity(parent, param, row, index) {
                ...
                if (param.isCheck) {
                    row.id = row.id + ' ' + param.addressName
                    // console.log(11111, this.newList)
                } else {
                    if (row.id.indexOf(param.addressName) !== -1) {
                        row.id = row.id.replace(param.addressName, ' ')
                    }
                }
                /*--------------------------------------------------------------------*/
                if (param.isCheck) {
                ...
                /*-------------------------------------------------------------------*/
                // console.log(123, row.dataList)
                // row.freightAddress = row.dataList
                row.origin_list = row.dataList
            },
            //点击编辑按钮
            showCheckedArea(row, index) {
                // console.log(22,this.selectedArr)
                // console.log(33,row.rowSelectArr)
                const newArr = JSON.parse(JSON.stringify(this.freightAddress))
                ...
                const secondArr = JSON.parse(JSON.stringify(this.freightAddress))
            ...
            add() {
                ...
                this.$set(newValue, 'origin_list', this.freightAddress)
                this.$set(newValue, 'id', '')
                this.$set(newValue, 'dataList', [])
            ...
            handleCheckProvince2(param, row, index) {
                ...
                //全选
                if (!param.isCheck) {
                    this.selectedArr5.push(param.addressId)
                    //当前行选中的id
                    row.rowSelectArr.push(param.addressId)
                    if (param.childAddress.length) {
                        param.childAddress.forEach(item => {
                            this.selectedArr5.push(item.addressId)
                            row.rowSelectArr.push(item.addressId)
                        })
                    }
                } else {
                    const index = this.selectedArr5.indexOf(param.addressId) //删除全部选中的id
                    const num = row.rowSelectArr.indexOf(param.addressId) //删除当前行的id
                    this.selectedArr5.splice(index, 1)
                    if (num != -1) {
                        row.rowSelectArr.splice(num, 1)
                    }
                    if (param.childAddress.length) {
                        param.childAddress.forEach(item => {
                            // 删除父级id
                            let index2 = this.selectedArr5.indexOf(item.addressId)
                            this.selectedArr5.splice(index2, 1)
                            let num2 = row.rowSelectArr.indexOf(item.addressId)
                            if (num2 != -1) {
                                row.rowSelectArr.splice(index2, 1) //删除子级id
                            }
            ...
            handleCheckCity2(parent, param, row, index) {
                ...
                if (param.isCheck) {
                    ...
                    const index3 = this.selectedArr5.indexOf(param.addressId) //删除选中的id
                    const num3 = row.rowSelectArr.indexOf(param.addressId) //删除当前行的id
                    this.selectedArr5.splice(index3, 1)
                    if (num3 != -1) {
                        row.rowSelectArr.splice(num3, 1)
                    }
                }
                // console.log(3344,this.selectedArr5,row.rowSelectArr)
                /*-------------------------------------------------------------------*/
                // console.log(123, row.dataList)
                // row.freightAddress = row.dataList
                row.origin_list = row.dataList
            ...
            showCheckedArea2(row, index) {
                ...
                if (this.selectedArr5.length) {
                    this.selectedArr5.forEach(item => {
