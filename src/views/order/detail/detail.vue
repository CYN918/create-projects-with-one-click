  ...
  <div class="margin">
    ...
    <div class="mcard">
      <el-row class="operate mbottom">
          <el-col :span="21" class="detail" v-if="tableData3[0]">
      ...
      <!-- <el-table :data="tableData8" border  stripe>
        <el-table-column label="名称" prop="fieldName" align="center"></el-table-column>
        <el-table-column label="内容" prop="fieldValue" align="center"></el-table-column>
      </el-table> -->
      <table style="border-collapse: collapse;border: 1px solid #EBEEF5;width: 100%;">
        <tr v-if="this.tableData8.length == '0'" style="border: 1px solid #EBEEF5;height: 48px;line-height: 48px;font-size: 14px;">
          <th style="border: 1px solid #EBEEF5;height: 48px;">暂无数据</th>
        </tr>
        <tr v-else v-for="item in tableData8" style="border: 1px solid #EBEEF5;height: 48px;line-height: 48px;font-size: 14px;">
          <th style="border: 1px solid #EBEEF5;height: 48px;">{{item.fieldName}}</th>
          <th style="border: 1px solid #EBEEF5;height: 48px;">{{item.fieldValue}}</th>
        </tr>
      </table>
    </div>
...
<script>
import { jicaiGetByidorder } from '@/api/order/index'
export default {
    data () {
    return {
      sysuserType:0, //角色状态
  ...
  methods: {
      ...
      _getByidorder(){
        jicaiGetByidorder(this.orderId).then(res => {
          if(res.data.orderGoodsList != null){
          ...
          this.tableData7.forEach((item, index) => {
            if(item.logisticsInfoList != null){
          ...
          this.tableData8 = res.data.fieldParams;
        })
      },
      back() {
      this.$router.go(-1);
