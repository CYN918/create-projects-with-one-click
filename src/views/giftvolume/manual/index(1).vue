...
<script>
import { reqBookList, topicdelete } from "@/api/giftvolume/index";
...
export default {
  ...
  methods: {
    ...
    // 分页查询所有主题
    async reqBookList() {
      const res = await reqBookList(this.form);
      console.log(res)
      if (res.code === 200) {
    ...
    addgifttype() {
      this.$router.push({ name: "gv_addmanual" });
    },
    editBrand(val) {
      this.$router.push({ name: "gv_addmanual", query: { id: val } });
    }
  },
};
</script>
