			...
			<el-table :data="tableData" border style="width: 100%" stripe>
				...
				<el-table-column   label="营业执照" align="center" width="140">
					<template slot-scope="scope">
						...
						<div>
							<el-button type="primary" size="small"  @click="open(scope.row.extend.businessLicense)">点击查看大图</el-button>
						</div>
					</template>
				</el-table-column>
				<el-table-column  label="一般纳税人资格证" align="center" width="140" >
					<template slot-scope="scope">
						...
						<div>
							<el-button type="primary" size="small"  @click="open(scope.row.extend.taxpayerQualification)">点击查看大图</el-button>
						</div>
					</template>
				</el-table-column>
				<el-table-column  prop="name"  label="品牌授权书" align="center" width="140">
					<template slot-scope="scope">
						...
						<div>
							<el-button type="primary" size="small"  @click="open(scope.row.extend.brandAuthUrl)">点击查看大图</el-button>
						</div>
					</template>
				</el-table-column>
				<el-table-column  prop="name"  label="授权链" align="center" width="140">
					<template slot-scope="scope">
						<img :src="getFirstUrl(scope.row.extend.authorizationChain)" alt="" class="img_size">
						<div>
							<el-button type="primary" size="small"  @click="open(scope.row.extend.authorizationChain)">点击查看大图</el-button>
						</div>
					</template>
				</el-table-column>
	...
	export default {
		...
		methods: {
			...
			detail(auditId, supplierId, auditStatus, state) {
				this.$router.push({name: "mysupplierDetail", query: {supplierId, state, auditId, auditStatus}});
			},
			editor(id) {
				this.$router.push({name: "mysupplieredit", query: {id: id}});
			},
			add() {
