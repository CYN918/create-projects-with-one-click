  ...
  <div class="margin mcard">
    ...
    <el-dialog title="设置新密码" :visible.sync="dialogPasswordVisible" width="600px">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="handleResetPassword" :loading="resteLoading">确 定</el-button>
      </span>
    </el-dialog>
...
import { getSysuserByid, submitData } from "@/api/setting";
...
export default {
  ...
  methods: {
    ...
    btnReset() {
      this.$prompt("请填写原密码以验证账户", "验证原密码", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        inputType: "password",
        inputPattern: /^\S/,
        inputErrorMessage: "请填写原密码以验证账户"
      })
        .then(({ value }) => {
    ...
    handleResetPassword() {
      this.$refs.formPassword.validate(valid => {
        if (valid) {
          ...
          resetPassword(this.formPassword.pass).then(res => {
