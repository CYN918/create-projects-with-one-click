<template>
  <div id="Offer">
          ...
          <div class="mail-wrapper">
            ...
            <div class="search-base">
              <div class="mbottom">
                <div class="goods-fitter">
                  <div class="mcard" style="padding-bottom:0">
                    <el-table :data="goodsList.quotes" border style="width: 100%" stripe 
                    ref="elTables1"  class="el1" @selection-change="AhandleSelectionChange">
              ...
              <div class="mbottom">
                <div class="goods-fitter">
                  <div class="mcard" style="padding-bottom:0">
                    ...
                    ref="elTables2" class="el2" @selection-change="BhandleSelectionChange">
                    <el-table-column type="selection" align="center"></el-table-column>
                      <el-table-column label="序号" type="index" align="center" width="50"></el-table-column>
                      <el-table-column
                        prop="commonContactName"
    ...
    <!-- 添加供应商1 -->
    <el-dialog :visible.sync="chooseVisible1" width="70%" :modal="true" :append-to-body="false" :before-close="addClosing1">
      <div class="mbottom">
        <div class="goods-fitter">
          <el-form label-width="100px" size="small">
            <div class="search-wrapper">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="danger" @click="addClose1" size="small">关 闭</el-button>
    ...
    <!-- 删除供应商1 -->
     <el-dialog :visible.sync="deleteVisible1" width="20%" title="提示" style="top:20%">
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="cancelGoods1()" size="small">确 定</el-button>
        </span>
      </el-dialog>
     <!-- 删除供应商2 -->
     <el-dialog :visible.sync="deleteVisible2" width="20%" title="提示" style="top:20%">
        <div class="mail-fitter" style="padding:20px 0;background:#fff;">
            <div class="search-base">
                  <div class="goods-fitter">
                    <el-row>
        ...
        <span slot="footer" class="dialog-footer">
          <el-button type="danger" @click="deleteVisible2 = false" size="small">取 消</el-button>
          <el-button type="primary" @click="cancelGoods2()" size="small">确 定</el-button>
        </span>
      </el-dialog>
  </div>
</template>
...
import {purchaseHistory,offerHistory,saveInquiry,deleteInquiryPrice,purchaseListGoods} from '@/api/purchase/index'
...
export default {
  data() {
    return {
      ...
      deleteVisible1:false,//删除供应商1
      deleteVisible2:false,//删除供应商2
      offerList:[],//历史报价
      purchaseList:[],//历史采购
      rowData:'',//获取列表数据
      deIndex:'',//删除对应商品的下表
  ...
  props: ["editVisibles", "goodsPage","sigdId"],
  created() {},
  watch: {
    editVisibles(val) {
  ...
  methods: {
    ...
    editInquirys() {
      ...
      saveInquiry(this.goodsList).then(res=>{
        if(res.code===200){
            throttleTipPop(this, "success", "供应商报价成功", 2000);             
             this.editVisible = false;
...
/*         arr[a].goodsQuantity = ''; */
        ...
        arr[a].taxCostPrice='0'; //含税价
        arr[a].notaxCostPrice='0';//不含税价
        arr[a].totalPrice='0';
        arr[a].deliveryTime='';
...
/*         arr[a].goodsQuantity = ''; */
    ...
    //删除弹框1
    deleteGoods1() {
        if(this.quoteId1!==''||this.deIndex1!==''){
    ...
    cancelGoods1(){
        ...
        obj.quoteId =this.quoteId1;
         deleteInquiryPrice(obj).then(res=>{
          if(res.code===200){
            this.goodsList.quotes.splice(this.deIndex3, 1);
            throttleTipPop(this, "success", "删除操作成功", 2000);
             this.quoteId1 = '';
             this.$refs.elTables1.clearSelection();
     ...
     AhandleSelectionChange(val) {
       ...
       if (val.length > 1) {
        ...
        this.$nextTick(function () {
          ...
          this.$refs.elTables1.clearSelection();      
          this.$refs.elTables1.toggleRowSelection(newArr);
        })
      }

      if(val[0]){
        if(val[0]&&val[0].quoteId){
          this.deIndex1 = '';
          this.quoteId1 =val[0].quoteId; 
          
          val.forEach((val, index) => {
      　　　　　　this.goodsList.quotes.forEach((v, i) => {
      　　　　　　　// id 是每一行的数据id
                  if(val.quoteId == v.quoteId){
                    this.deIndex3 = i
                    console.log(i);
                  }
                })
              }) 
        }else if(val[0]&&!this.deIndex1){
          this.quoteId1 = '';
    ...
    //删除弹框2
    deleteGoods2() {
        if(this.quoteId2!==''||this.deIndex2!==''){
    ...
    cancelGoods2(){
        ...
        let obj ={}
        obj.sigdId = this.sigdId;
        obj.quoteId =this.quoteId2;
         deleteInquiryPrice(obj).then(res=>{
          if(res.code===200){
            this.goodsList.similarQuotes.splice(this.deIndex4, 1);
            throttleTipPop(this, "success", "删除操作成功", 2000);
             this.quoteId2 = '';
             this.$refs.elTables2.clearSelection();
             this.deleteVisible2 = false;
          }
        })
      }else{
         if(this.deIndex2!==""){
             this.goodsList.similarQuotes.splice(this.deIndex2, 1);
             throttleTipPop(this, "success", "删除操作成功", 2000);
             this.deIndex2 = '';
             this.$refs.elTables2.clearSelection();
             this.deleteVisible2 = false;
     ...
     BhandleSelectionChange(val) {
       ...
       if (val.length > 1) {
        ...
        this.$nextTick(function () {
          ...
          this.$refs.elTables2.clearSelection();      
          this.$refs.elTables2.toggleRowSelection(newArr);
        })
