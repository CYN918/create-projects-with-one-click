<template>
  <div id="toInvioce">
    <el-dialog width="73%" :visible.sync="editVisible" :before-close="createClosing">
     ...
     <div class="mbottom">
        <div class="goods-fitter">
          <el-form ref="infoForm" :model="pageData" :rules="rules" label-width="150px" size="small">
            <div class="search-wrapper">
              <div class="mbottom">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="updateInvoice()" size="small">确 定</el-button>
      </span>
    </el-dialog>
      <!-- 上传预览 -->
...
<script>
import { updateInvoiceInfo } from "@/api/purchase/index";
...
export default {
  ...
  props: ["editVisibles","firstLevel",'companyData','invoiceData'],
  watch: {
    editVisibles(val) {
    ...
    invoiceData(val){
      this.pageData = val;
         if(this.files.length===0&&this.pageData.fileInvoiceUrl!==null&&this.pageData.fileInvoiceUrl!==''){
              this.files.push({url:this.pageData.fileInvoiceUrl}); 
        }
  ...
  methods: {
    ...
    createClose() {
      ...
      this.$confirm('您确认要关闭吗？')
                        ...
                        this.files = [];
                        this.$emit("createClose3", this.editVisible);
                })
                .catch(_ => {});
    },
    createClosing() {
      ...
      this.$confirm('您确认要关闭吗？')
                       ...
                       this.editVisible = false;
                       this.files = [];
                       this.$emit("createClosing3", this.editVisible);
                })
                .catch(_ => {});
    },
    btn1() {
    ...
    updateInvoice(){
        if(this.form.imgUrl!==''){
            this.pageData.fileInvoiceUrl = this.form.imgUrl;
        }
      
      updateInvoiceInfo(this.pageData).then(res=>{
           if(res.code===200){
               throttleTipPop(this, "success","编辑收票内容成功！", 2000);
               this.files = [];
               this.editVisible = false;
               this.$emit('updateIovoice1',this.editVisible);
           }
      })
    },
      handleProgress(){
       if(this.pageData.fileInvoiceUrl!==''){
            this.pageData.fileInvoiceUrl === '';
            this.hiddenIndex = 1;
         }
