    ...
    <div class="mcard" style="border:1px solid #d7e0f1">
      <el-table :data="inPage" border @selection-change="handleSelectionChange" stripe>
        ...
        <el-table-column label="不含税单价" prop="name" align="center">
         ...
         <el-table-column label="含税金额" prop="name" align="center">
              <template slot-scope="scope">
                    {{scope.row.totalAmount}}
              </template>
         </el-table-column>
      </el-table>
      <div class="total" style="margin-left:81%;margin-top:20px">
...
import {InvoiceGoodsList} from "@/api/purchase/index";
export default {
  data () {
    return {
      form: {
  ...
  computed: {
      ...
      if(_this.inPage.length>0){
          for(var i = 0;i<_this.inPage.length;i++){
            if(!isNaN(_this.inPage[i].totalAmount)){
              totals+=parseFloat(_this.inPage[i].totalAmount)
            }else{
              totals = 0
            }
      }
