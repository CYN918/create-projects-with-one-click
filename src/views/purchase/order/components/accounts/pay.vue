<template>
    <div id="addPurchasing">
        <el-dialog width="75%" :visible.sync="payVisible" :before-close="createClosing">  
        <div class="mbottom">
          <div class="goods-fitter">
            <el-form label-width="100px" size="small">
              <div class="search-wrapper">
                <h4 class="title">付款单-付款</h4>
              </div>
            </el-form>
          </div>
        </div>
       ...
       <div class="mbottom">
                      ...
                      <el-col :span="12">
                        <el-form-item label="付款原因：" prop="storageMan">
                           <el-radio-group v-model="payData.payReason" @change="depoit" disabled>
                                ...
                                <input class="other" disabled v-show="this.otherIndex===0" v-model="form.otherReason"/>
                                <input class="other"  v-show="this.otherIndex===1" v-model="form.otherReason"/>
                        ...
                        <el-col :span="6">
                            ...
                            <el-col :span="6">
                                <el-form-item label="资金账户：">
                                    <el-col :span="23">
                                        <el-select v-model="payData.payAcountId" @change="chooseAcount">
                                                <el-option
                     ...
                     <el-row>
                          <el-col :span="12">
                            <el-form-item label="指定往来科目：" prop="logisticsTel">
                              <el-col :span="24">
                                <el-input v-model="payData.currentAccount"></el-input>
                              </el-col>
                            </el-form-item>
                          </el-col>
                        <el-col :span="12">
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="pay" size="small">支 付</el-button>
        </span>
      </el-dialog>
         <!-- 上传预览 -->
...
import {saveStorage,payPayment} from '@/api/purchase/index'
...
export default {
    ...
    data(){
        return{
            ...
            tableData:[
                 ...
                 options4:[
                    {value:1546546,label:'百事达'},
                    {value:2123123,label:'华美贵'},
                    {value:3546544,label:'天翼'},
                    {value:4123123,label:'华通'},
    ...
    props:["payVisibles","payList","accountData"],
    watch: {
        payVisibles(val) {
          this.payVisible = val; 
    ...
    //关闭页面
     createClose() {
      ...
      this.$confirm('您确认要关闭吗？')
                      ...
                      this.showIndex = 0;
                      this.otherIndex =0;
                      this.$emit("createClose7", this.payVisible);
                })
                .catch(_ => {});
    ...
    createClosing() {
      ...
      this.$confirm('您确认要关闭吗？')
                      ...
                      this.showIndex = 0;
                      this.otherIndex =0;
                      this.$emit("createClosing7", this.payVisible);
                })
                .catch(_ => {});
    ...
    handleAvatarSuccess(file) {     
        ...
        this.payData.annex = file.data.domain + file.data.filePath;
      },
      ...
      //取得付款账号
       chooseAcount(data){
          let obj = {};
          obj = this.accountData.find((item)=>{
            return item.accId === data;
        });
        this.payData.payAcountNo = obj.accountNickName;
      },
      //支付
      pay(){
          let obj ={};
          obj.billNo = this.payData.billNo;
          obj.currentAccount=this.payData.currentAccount, //往来科目
          obj.capitalAccount=this.payData.capitalAccount, //资金科目
          obj.annex=this.payData.annex,//银行付款凭证
          obj.payAcountId=this.payData.payAcountId,//收款账号id,财务管理-设置管理-资金账号
          obj.payAcountNo=this.payData.payAcountNo,//收款账号
          payPayment(obj).then(res=>{
              if(res.code===200){
                  throttleTipPop(this, "success","付款成功！", 2000);
                  this.payVisible = false;
                  this.$emit('payPayment1',this.payVisible);
              }
          })
      },
    createStorage(){
