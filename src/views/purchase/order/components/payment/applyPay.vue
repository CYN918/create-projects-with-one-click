...
import {saveStorage,savePurchasePayment} from '@/api/purchase/index'
...
export default {
    ...
    watch: {
        ...
        payList(val){
          this.payData = val;
          this.total = this.payData.details.length;
        },
     /*    'payData.applyAmount':{
          handler(val){
            this.bIndex = 0;
  ...
  /*   getCount(val){
    ...
    savePayment(){
       ...
       savePurchasePayment(this.payData).then(res=>{
         if(res.code===200){
           throttleTipPop(this, "success","保存付款申请成功！", 2000);
           this.addVisible = false;
           this.$emit('saveAppPay1',this.addVisible);
         }
       })
    },
    //操纵定金的显示隐藏
