<template>
		<div class="margin">
				...
				<div class="card">
						<ul class="list">
								<li class="item" v-for="(item1, index1) in list"
										:key="index1">
										<div class="item_one">
												<!-- 遮罩层盒子 -->
												<div v-show="seen1&&index1==current1" class="index-show">
														<i class="iconfont icon-bianji iconfont_style_one"></i>
														<i class="iconfont icon-shanchu iconfont_style_two"></i>
												</div>
												<!-- 顶部logo -->
												<div class="word_line">
												...
												<el-button class="word_two" @click="upGread(item1.gradeNo)">升级会员服务</el-button>
										</div>
										<div v-if="item1.seen">
...
<script>
    import {deleteGrade, editGradeList, getGradeList} from "@/api/service";
    ...
    export default {
        ...
        created() {
			this.getList();
			this.gradeIndex = this.$route.query.gradeNo
        },
        computed: {},
        methods: {
			back() {
                ...
                getGradeList().then(res => {
                    if (res.code == 200) {
						res.data.forEach( item =>{
						  if( item.gradeNo == this.gradeIndex ){
							 this.list.push(item)
							 this.content = '服务中心>我的服务>'+ item.gradeName
						  }
						})
...
<style scoped>
  .search-wrapper >>> .el-page-header__title{
          margin-top: 3px;
  }   
</style>
