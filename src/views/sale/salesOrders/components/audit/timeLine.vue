                                      ...
                                      <el-form ref="infoForm"  label-width="60px" size="small">  
                                        ...
                                        <div class="time" :class="item.end===0?'active':''">
                                            ...
                                            <p class="timeFont" :class="item.result=='审批不通过'?'showColor':''"  v-show="item.permission!==1"> 审批结果：{{item.result}}      
                                                ...
                                                <el-button size="small" type="danger" style="float:right;" @click="urgent(index)" class="run" :class="showIndex===index?'showActive':''"
                                                >加急处理</el-button>
                                            </p>
...
<script>
import {auditUrgent} from "@/api/sale/index"
...
export default {
    ...
    props:['goVisibles','auditData','orderId'],
    watch:{
        goVisibles(val){
            this.goVisible = val
        },
    ...
    methods:{
            ...
            urgent(index){
                ...
                obj.orderId = this.orderId1;
                auditUrgent(obj).then(res=>{
                    if(res.code===200){
