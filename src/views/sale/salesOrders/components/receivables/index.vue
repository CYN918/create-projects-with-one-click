      ...
      <div class="margin0">
        ...
        <div class="mcard" style="border:1px solid #d7e0f1">
          ...
          <el-table :data="payList" border @selection-change="handleSelectionChange" 
            ...
            <el-table-column label="已收金额" prop="name" align="center">
                        <template slot-scope="scope">
                              {{scope.row.receivedAmount}}
                      </template>
            </el-table-column>
            <el-table-column label="未收金额" prop="name" align="center">
                        <template slot-scope="scope">
                              {{scope.row.unpaidAmount}}
                      </template>
            </el-table-column>
            <el-table-column label="开票状态" prop="name" align="center">
                        <template slot-scope="scope">
        ...
        :saveNo="saveNo"  
        :payData="payData"
          @createCloseA="createClose4"
            @createClosingA="createClosing4"
            @payment="payment1">
...
import {getSalePayList,getSalePayGoodsList,getSalePagePayList,findAccountInfo} from '@/api/sale/index'
...
export default { 
  ...
  methods: {
    ...
    //新增付款
    generate(row){
      this.saveNo = row.saleOrderNo;
      findAccountInfo().then(res=>{
        if(res.code===200){
          this.payData = res.data.pageList;
        }
      })
