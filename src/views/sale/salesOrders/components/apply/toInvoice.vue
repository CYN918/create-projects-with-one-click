              ...
              <div class="search-base">
                ...
                <el-row>
                 ...
                 <el-row>
                  ...
                  <el-col :span="12">
                    <el-col>
                      <el-form-item label="合计金额：" prop="invoiceAmount">
                        <el-col :span="19">
                          <el-input
...
import {findLocation,toApplyInvioce,uploadingInvoice,downloadInvoice} from '@/api/sale/index';
...
export default {
  ...
  created() {},
  props: ["addVisibles","invoiceData","firstLevel",'companyData',"cityData","areaData"],
  watch: {
    addVisibles(val) {
      this.addVisible = val;
    },
      ...
      handler(newVal, oldVal) {
        for(let i=0;i<newVal.length;i++){
          if(parseFloat(newVal[i].goodsCount)>=0&&parseFloat(newVal[i].goodsPrice)>=0){
              newVal[i].totalAmount = parseFloat(newVal[i].goodsCount) * parseFloat(newVal[i].goodsPrice)
          }
        }
    },
    deep: true
    },
    'pageData.openInvoiceNum':{
      handler(newVal){
        if(newVal>=10){
              throttleTipPop(this, "warning", "请使用导入发票功能", 2000);
        }
      }
    }
  },
     ...
     /*  this.pageData.invoiceAmount = this.total1; */
      ...
      this.pageData.opFlag = 1;
        if(!this.pageData.invoiceType||
         ...
         if(this.pageData.openStatus!==1){
                 throttleTipPop(this, "warning", "开票状态请选择为已开票", 2000);
                 return ;
          }else{
            toApplyInvioce(this.pageData).then(res=>{
                  if(res.code===200){
                    throttleTipPop(this, "success", "申请开票成功", 2000);
                    this.addVisible =false;
