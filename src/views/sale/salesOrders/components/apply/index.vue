<template>
  <div class="margin0">
    ...
    <div class="mcard" style="border:1px solid #d7e0f1">
      ...
      >
        ...
        <el-table-column label="开票内容" prop="name" align="center">
          <template slot-scope="scope">{{scope.row.invoiceContent}}</template>
        </el-table-column>
        <el-table-column label="开票类型" prop="name" align="center">
          <template slot-scope="scope">{{scope.row.invoiceType | invoice}}</template>
        </el-table-column>
        ...
        <el-table-column label="申请开票金额" prop="name" align="center">
          <template slot-scope="scope">{{scope.row.totalInvoiceAmount}}</template>
        </el-table-column>
        <el-table-column label="未开票金额" prop="name" align="center">
          <template slot-scope="scope">{{scope.row.notInvoiceAmount}}</template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            <div class="line-div">
              <div
                ...
                v-permit="'SaleOrder:to_open_invoice'"
              >
                <div class="i-div">
                  <img src="@/common/images/icon/发票.png" alt class="operation_img" />
                </div>
                <span style="word-break: keep-all;">去开票</span>
              </div>

              <div
                class="small-Idiv"
                @click="editInvoice(scope.row)"
                v-permit="'SaleOrder:edit_invoice'"
              >
                <div class="i-div">
    ...
    <InvoiceDetail
      ...
      :areaData="areaData"
      :companyData="companyData"
      @createCloseD="createClose5"
      @createClosingD="createClosing5"
      @editInvioce1="editInvioce2"
    ></InvoiceDetail>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    _getApplyInvoicesList() {
      getApplyInvoicesList(this.form).then(res => {
    ...
    generate(row) {
      ...
      getInvioceData(row.invoiceId).then(res => {
        if (res.code === 200) {
          ...
          findLocation(this.invoiceData.addressInfoVo.provinceId).then(res => {
            this.cityData = res.data.list;
          });
          findLocation(this.invoiceData.addressInfoVo.cityId).then(res => {
