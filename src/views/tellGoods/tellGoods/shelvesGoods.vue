								...
								<el-form ref="form" :model="form" label-width="120px" size="small">
										<div class="search-wrapper">
												...
												<el-row :gutter="10" style="margin-left: -5px;margin-top: -14px;margin-bottom: 14px;">
														<el-col :span="24">
																<el-button type="primary" size="mini" @click="uploadGoods(false)">批量上架</el-button>
														</el-col>
												</el-row>
												<!--表格-->
												<el-table
														...
														<el-table-column style="color:red;" align="center" prop="goodsThumbnailImgUrl" label="图片"
																						 width="213">
														...
														<el-table-column style="color:red;" align="center" label="商品详情" width="250px">
																<template slot-scope="scope">
																		<div
																				style="display: flex;flex-direction: column;justify-content: center;align-items: flex-start">
																				<span>{{scope.row.goodsName}}</span>
																				<br>
																				<span>货号:{{scope.row.goodsSn}}</span>
																				<br>
																				<span>型号:{{scope.row.modelNumber}}</span>
																				<br>
																				<span>品牌:{{scope.row.brandName}}</span>
																		</div>
																</template>
														</el-table-column>
														<el-table-column align="center" prop="brand" label="规格尺寸">
																<template slot-scope="scope">
																		<ul style="display: flex;flex-direction: column;align-items: center;justify-content: center">
																				<li v-for="(item,index) in scope.row.specList">
																						<span>{{item.specName+':'+item.specValue}}</span>
																				</li>
																		</ul>
																</template>
														</el-table-column>
														<el-table-column align="center" label="商城价">
																<template slot-scope="scope">
																		<div
																				style="display: flex;flex-direction: column;justify-content: center;align-items: flex-start">
																				<span>{{'商场价:'+scope.row.shopPrice}}</span>
																				<br>
																				<span>{{'市场价:'+scope.row.marketPrice}}</span>
																				<br>
																				<span>{{'成本对比京东/天猫折扣价:'+scope.row.costDiscountProfit}}</span>
																				<br>
																				<span>{{'商城价对比京东/天猫折扣价:'+scope.row.shopDiscountProfit}}</span>
																		</div>
																</template>
														</el-table-column>
														<el-table-column align="center" label="操作" width="180px">
																<template slot-scope="scope">
																		<div class="small-Idiv" @click="checkDetail(scope.row.specList[0].goodsId)">
																				<div class="i-div">
																		...
																		<div class="small-Idiv"
																				 @click="uploadGoods(scope.row.superReportId)">
																				<div class="i-div huishouzhan">
																						<img src="@/common/images/icon/icon_contract.png" alt=""
																								 class="operation_img">
																				</div>
																				<span style="word-break: keep-all;">上架</span>
																		</div>
																</template>
														</el-table-column>
												</el-table>
...
<script>
    import {get_show_goods, get_show_goods2, getDetailGoods, up_goods,deleteTellGoods3} from '@/api/tellGoods/index'
    ...
    export default {
        ...
        computed: {
            ...mapState({
                handleListParams:state=>state.tellGoodsList.handleListParams
            })
        },
        methods: {
            //获取详情
           ...
           /* //页容量改变
            ...
            //查看详情??
            checkDetail(goodsId) {
                this.$router.push({name: 'goodsDetail2', query: {goodsId: goodsId}})
            },
            //删除
            deleteRow(index, rows) {
                this.$confirm('是否确定移除此行', '提示', {
                    ...
                    .then(async () => {
                        //删除请求
                        deleteTellGoods3(rows.superReportId).then(res=>{
                            rows.splice(index, 1);
            ...
            //上架商品---可以批量
            uploadGoods(supReportGoodsIds) {
                /*console.log(333,supReportGoodsIds)*/
                if (supReportGoodsIds) {
                    up_goods({supReportGoodsIds,reportId:this.reportId}).then(res => {
                        if (res.code == 200) {
                    ...
                    const newList=[]
                    this.handleListParams.forEach(item=>{
                        newList.push(item.superReportId)
                    })
                    // //批量上架操作,逗号隔开??
                    var str=newList.join(",")
                    console.log(555,str)
                    up_goods({supReportGoodsIds:str,reportId:this.reportId}).then(res => {
                        if (res.code == 200) {
                            this.$message.success('上架成功')
                        }
                    })
                    console.log(999,this.handleListParams)
                }
            },
            ...
            handleSelectionChange2(val) {
                console.log(444,val)
                if (val.length > 0) { // 表示有被选中的列
                    this.$store.commit('tellGoodsList/SET_tellGoodsList', val)

                } else {
                    this.$store.commit('tellGoodsList/SET_tellGoodsList', [])
                }
            },
            // 选中提示
            handleSelect2() {
