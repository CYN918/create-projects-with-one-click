    ...
    <div class="wrapper">
      ...
      <el-table
        ...
        <el-table-column label="库存(PCS)" align="center" width="120">
          <template slot-scope="scope">
            <p v-if="!pros[scope.$index]">{{scope.row.bgGoodsSkuReturnList[scope.row.index].goodsNumber}}</p>
            <el-input v-else type="number" min="0" v-model="scope.row.bgGoodsSkuReturnList[scope.row.index].goodsNumber"></el-input>
          </template>
        </el-table-column>
        <el-table-column label="供应商活动价" align="center" width="120">
          <template slot-scope="scope">
            <p v-if="!pros[scope.$index]">{{scope.row.bgGoodsSkuReturnList[scope.row.index].supplierActivityPrice}}</p>
            <el-input v-else type="number" min="0" v-model="scope.row.bgGoodsSkuReturnList[scope.row.index].supplierActivityPrice"></el-input>
          </template>
        </el-table-column>
        <el-table-column v-if="haveAuthority === 1" label="操作" align="center" width="180" fixed="right">
          <template slot-scope="scope">
            <div class="line-div small_top">
              <div v-if="!pros[scope.$index] " class="small-Idiv" @click="edit(scope.$index)">
                <div class="i-div">
                  <img src="@/common/images/icon/编辑.png" alt class="operation_img"/>
                </div>
                <span>编辑</span>
              </div>
              <div v-else-if="pros[scope.$index]" class="small-Idiv" @click="confirmEdit(scope.row, scope.$index)">
                <div class="i-div">
                  <img src="@/common/images/icon/编辑.png" alt class="operation_img"/>
                </div>
                <span>保存</span>
...
export default {
  ...
  methods: {
    ...
    async checkAuthority() {
      const res = await checkAuthority({type: 4, activityId: this.$route.query.activityId})
      if (res.code === 200) {
    ...
    async confirmEdit(row, index) {
      ...
      obj.skuParamList.forEach((item, index) => {
        ...
        item.supplierActivityPrice = row.bgGoodsSkuReturnList[index].supplierActivityPrice
        if (item.supplierActivityPrice < 0) {
          throttleTipPop(this, 'error', '供应商活动价设置不得少于0')
          return
        }
        if (item.activityGoodsNumber < 0) {
          throttleTipPop(this, 'error', '库存设置不得少于0')
          return
        }
      })
      arr.push(obj)
