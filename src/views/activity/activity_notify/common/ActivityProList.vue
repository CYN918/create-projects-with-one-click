  ...
  <div class="margin dealerlist">
    ...
    <div class="wrapper">
      ...
      <el-table
        ...
        <el-table-column v-if="haveAuthority3 === 1" label="一件代发价" align="center" width="100">
          <template slot-scope="scope">
        ...
        <el-table-column v-if="haveAuthority1 === 1" label="经销商结算价" align="center" width="100">
          <template slot-scope="scope">
            <span>{{scope.row.bgGoodsSkuReturnList[scope.row.index].distributorActivityPrice}}</span>
          </template>
        </el-table-column>
        <el-table-column v-if="haveAuthority2 !== 0" label="平台结算价" align="center" width="100">
          <template slot-scope="scope">
            <div v-if="haveAuthority2 === 1">
              <p>{{scope.row.bgGoodsSkuReturnList[scope.row.index].platformSettlementPrice}}</p>
            </div>
            <div v-if="haveAuthority2 === 2">
        ...
        <el-table-column v-if="haveAuthority4 === 1" label="平台活动价" align="center" width="100">
          <template slot-scope="scope">
        ...
        <el-table-column label="操作" align="center" width="202" fixed="right">
          <template slot-scope="scope">
            <div class="line-div small_top">
              <div v-if="(haveAuthority4 === 1 || haveAuthority2 !== 0) && !pros[scope.$index]" class="small-Idiv" @click="edit(scope.$index)">
                <div class="i-div">
                  <img src="@/common/images/icon/编辑.png" alt class="operation_img"/>
                </div>
                <span>编辑</span>
              </div>
              <div v-else-if="(haveAuthority4 === 1 || haveAuthority2 !== 0) && pros[scope.$index]" class="small-Idiv" @click="confirmEdit(scope.row, scope.$index)">
                <div class="i-div">
                  <img src="@/common/images/icon/编辑.png" alt class="operation_img"/>
                </div>
                <span>保存</span>
              ...
              <div class="small-Idiv" @click="recycleBinfunc(scope.row.activityGoodsId)">
                <div class="i-div huishouzhan">
                  <img src="@/common/images/icon/下架_上架.png" alt class="operation_img"/>
                </div>
                <span>下架</span>
              </div>
            </div>
...
import { checkAuthority, updataActivityProExamineStatus, reqActivityProList, reqActivityProClassify, updateActivityPro, deleteActivityPro } from '@/api/activity/index'
...
export default {
  ...
  methods: {
    ...
    async checkAuthority1() {
      const res = await checkAuthority({type: 6, activityId: this.$route.query.activityId})
      if (res.code === 200) {
        this.haveAuthority1 = res.data
      }
    },
    async checkAuthority2() {
      const res = await checkAuthority({type: 7, activityId: this.$route.query.activityId, position:4})
      if (res.code === 200) {
    ...
    // 审核活动
    examineActivity(activityGoodsId) {
      this.examineParams.activityGoodsId = activityGoodsId
