      ...
      <div>
        ...
        >
          <div v-for="(item,index) in dataArr" :key="index">
            <el-form-item :label="item.settingInfoList[0].btnType==='multiple_input_number'?'':item.settingName+'：'" prop="name" :label-width="item.settingInfoList[0].btnType==='multiple_input_number'?'0':'400px'">
  ...
  methods: {
      ...
      data.forEach(item => {
        ...
        item.settingInfoList.forEach(val => {
          if (val.checkValue == "true" && val.btnType == "radio") {
            ...
            item.input = val.checkValue;
          } else if (val.btnType == "multiple_input_number") {
    ...
    async submit() {
      ...
      Arr.forEach(item => {
        ...
        item.keyValues = [];
        if(item.settingInfoList[0].btnType === "multiple_input_number"){
          item.settingInfoList.forEach(data => {
            let obj = {}
            obj[data.key] = data.input
           ...
           item.radioArr.forEach((val,index) => {
            let obj = {};
            item.input != ""
              ? (obj[item.settingInfoList[index].key] = item.input)
