  ...
  <div class>
    <div class="mcard">
      ...
      <div>
        ...
        <div class="btn_div">
          <el-button type="danger" size="small" @click="resert" v-permit="'submission:reset'">重 置</el-button>
          <el-button type="primary" size="small" @click="submit" v-if="isShow" v-permit="'submission:save'">确 定</el-button>
          <el-button type="primary" size="small" @click="saveChange" v-else v-permit="'submission:save'">保 存 修 改</el-button>
        </div>
      </div>
    </div>
  </div>
...
<script>
import { getfindbytype , ediotfindbytype,submission,submissionfindbytype,saveSubmission} from "@/api/systemsettings/index";
    ...
    saveChange(){
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          let obj = JSON.parse(JSON.stringify(this.ruleForm))
          obj.enable == '0'?obj.enable=true:obj.enable=false
          submissionfindbytype(obj).then(res => {
            if(res.code == 200){
              throttleTipPop(this,'success','操作成功')
              this._getfindbytype();
            }
