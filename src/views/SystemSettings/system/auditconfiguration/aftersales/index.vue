        ...
        >
          <div class="top_div">
            <span
              ...
              :key="key"
              :class="index === item.value?'select_span':''"
          ...
          <!-- <el-form-item label="" label-width="180px">
            <el-dropdown  @command="handleCommand">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="100">退货/退款</el-dropdown-item>
                <el-dropdown-item command="101">维修</el-dropdown-item>
                <el-dropdown-item command="102">换货</el-dropdown-item>
                <el-dropdown-item command="103">破损补寄</el-dropdown-item>
                <el-dropdown-item command="104">丢件补寄</el-dropdown-item>
                <el-dropdown-item command="105">补寄配件</el-dropdown-item>
                <el-dropdown-item command="106">补偿费用</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </el-form-item>-->
          <div class="content_div">
            <div class="top_div2">
              <span :class="typespan==1?'color_span':''" @click="changetype(1)">售后单审核</span>
              <span
                :class="typespan==2?'color_span':''"
                @click="changetype(2)"
                v-if="index != 100 && index != 106"
              >财务审核</span>
              <span
                :class="typespan==2?'color_span':''"
                @click="changetype(2)"
              ...
              >退款审核</span>
              <span
                :class="typespan==3?'color_span':''"
                @click="changetype(3)"
                v-if="index == 100 || index == 106"
              >财务退款</span>
            </div>
            <el-form-item label="售后订单审核：" prop="name" v-if="typespan != 3">
              <el-radio-group v-model="ruleForm.enable">
                <el-radio label="0">需要审核</el-radio>
                <el-radio label="1">不需要审核</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="审核对象类型：" prop="name">售后订单 - {{text}}</el-form-item>
            <el-form-item label="审核模式：" prop="name" v-if="typespan != 3">
            ...
            <div v-for="(item,index) in ruleForm.settingList" :key="index">
              ...
              <div class="dele_div">
                <div class="dete_icon" @click="deterecord(index)" v-if="typespan != 3">
                ...
                >
                  <div style="display:flex">
                    ...
                    >
                      <treeselect
                        ...
                        :key="index"
                        placeholder="请选择"
                        v-model="val.companyId"
                      />
                    </el-form-item>
                    <el-form-item style="width: 300px;" label="部门：" props='organizeId'
                    :rules="{ required: true, message: '请选择公司名称', trigger: 'change' }"
                    >
                      <treeselect
                        style="width:218px"
                    ...
                    <el-form-item label="审核人：">
                      <el-select v-model="val.userId" clearable @change="select(val, 'user')">
                        <el-option
                  ...
                  <div class="btm_div">
                    <el-form-item  style="display: inline-block;"  label="审核权限：" label-width="120px" v-if="typespan != 3"
                    >
                    ...
                    <span
                      class="add"
                      v-if="index2 == item.auditRoleList.length-1 && typespan != 3"
...
export default {
  ...
  methods: {
    ...
    async _handleSelect(val, index, index2, clear = false) {
      ...
      if (clear) {
        console.log('测试')
        this.ruleForm.settingList[index].auditRoleList[index2].userId = '';
        this.verifyRequire();
      }
      if (true) {
    ...
    addsales() {
      ...
      this.showUserList(
        { organizeId: def },
        this.ruleForm.settingList.length - 1,
        0
      );
    },
    select(val, type = "") {
    ...
    async _findAmountAuditByType() {
      let $this = this;
      const numobj =
        JSON.stringify(this.type) + "0" + JSON.stringify(this.typespan);
      let { data } = await findAmountAuditByType(numobj);
      ...
      data.settingList.forEach((val, index1) => {
        ...
        val.auditRoleList.forEach(item => {
          ...
          item.companyId = this.companyArr[0].id;
    ...
    verifyRequire() {
      ...
      for (let i = 0; i < arr.length; i++) {
        for (let j = 0; j < arr[i].auditRoleList.length; j++) {
          if (
            arr[i].auditRoleList[j].showradio ||
            arr[i].auditRoleList[j].showOrgan
          ) {
            return true;
          }
        }
