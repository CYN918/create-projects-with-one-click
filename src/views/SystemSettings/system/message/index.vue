          ...
          <el-form-item :label="item.settingName+'：'" prop="name">
            <div  v-if="item.settingInfoList[0].btnType==='radio'">
              <el-radio-group v-model="item.radioValue">
                <el-radio :label="data.key" v-for="(data,val) in item.settingInfoList" :key="val">{{data.text}}</el-radio>
              </el-radio-group>
            </div>
...
export default {
  ...
  methods:{
    async _findlistByparentcode(){
      const { data } = await findlistByparentcode('msg');
      data.forEach(item => {
        item.radioValue = ''
        item.radioArr = []
