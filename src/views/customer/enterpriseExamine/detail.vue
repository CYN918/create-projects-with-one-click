      ...
      <div class="mbottom">
        <div class="goods-fitter">
          <div class="search-wrapper1">
            <el-page-header @back="back" content="客户资料详情"></el-page-header>
          </div>
        </div>
      </div>
            ...
            <el-row>
              <el-col :span="5">
                <el-form-item label="企业名称:">
                  <span>{{form.enterprieName}}</span>
                </el-form-item>
              </el-col>
              <el-col :span="5">
                <el-form-item label="企业类型:">
                  <span>{{form.enterprieNature}}</span>
                </el-form-item>
              </el-col>
              <el-col :span="5">
                <el-form-item label="统一社会信用代码:">
                  <span>{{form.enterBussNo}}</span>
                </el-form-item>
              </el-col>
              <el-col :span="5">
            ...
            <el-row>
              ...
              <el-col :span="5">
                <el-form-item label="地址:">
                  <span>{{form.enterprieAddress}}</span>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              ...
              <el-col :span="5">
                <el-form-item label="创建时间:">
                  <span>{{this.auditDatas.createTime}}</span>
                </el-form-item>
              </el-col>
              <!-- <el-col :span="5">
            ...
            <el-row>
              ...
              <el-col :span="5">
                <el-form-item label="联系周期:" prop="customerPeriod">
                  <span>{{form.customerPeriod | timer }}</span>
                </el-form-item>
              </el-col> -->
            </el-row>
          </div>
      ...
      <div class="mbottom">
        <div class="goods-fitter">
          <!-- <div class="search-wrapper">
            ...
            <el-row>
              <el-col :span="5">
                <el-form-item label="联系地址:" prop="enterprieAddress">
                  <span>{{form.enterprieAddress}}</span>
                </el-form-item>
              </el-col>
              <el-col :span="5">
      ...
      <div class="mbottom">
        <div class="goods-fitter">
          <!-- <div class="search-wrapper">
            ...
            <el-row>
              <el-col :span="5">
                <el-form-item label="公司名称:" prop="enterprieName">
                  <span>{{form.enterprieName}}</span>
                </el-form-item>
              </el-col>
              <el-col :span="5">
              ...
              <el-col :span="5">
                <el-form-item label="注册地址:" prop="enterprieAddress">
                  <span>{{form.enterprieAddress}}</span>
                </el-form-item>
              </el-col>
            </el-row>
          ...
          <div class="margin">
            <div class="msetting-account-wrapper" style="margin-left: -20px;">
              <div class="tabs-header">
                <ul class="clearfix">
                  <li @click="tabIndex(0)" :class="currentIndex===0 ? 'active' : ''"
                      v-show="this.auditDetail!=''">当前审核详细</li>
                  <!-- <li @click="tabIndex(index+1)" :class="currentIndex===index+1 ? 'active' : ''" v-if="auditDetails.length >1"
                      v-for="(item,index) in this.auditDetails" :key="index">{{index==0?'首次审核':index+1+'级审核'}}</li> -->
                </ul>
              </div>
                  ...
                  <div class="mbottom">
                    <div class="goods-fitter">
                      <el-form label-width="260px" size="small">
                        <div class="search-wrapper">
                          <h4 class="title">审核明细({{ this.auditDatas.remark }})  <span style="padding-left: 50px;">发起时间: {{this.auditDatas.createTime}}</span></h4>
                          <div class="wrapper" v-if="this.auditDetail!=''">
                            <ul class="list">
                              <li class="item" v-for="(item,index) in this.auditDetail" :key="index">
                                <div style="text-align: center"
                                     :class="{'label word': item.taskResult==1, 'label label_2  word': item.taskResult==2, 'label label_3  word': item.taskResult == 0}"
                                >
                                  <i
                                      :class="item.taskResult==1?'el-icon-circle-check':'el-icon-circle-close'"
                                  ></i>
                                  <span class="t2" v-if="item.taskResult === 1">
                                    已通过
                                  </span>
                                  <span class="t2" v-if="item.taskResult === 2">
                                    未通过
                                  </span>
                                  <span class="t2" v-if="item.taskResult === 0">
                                    待审核
                                  </span>
                                </div>
                                <div class="word_line">
                                  <span class="t1 tt">审核人：</span>
                                  <span class="t1">{{item.userName}}({{item.organizeName||item.roleName||item.userName}})</span>
                                </div>
                                <div class="word_line">
                                  <span class="t1 tt">审核结果：</span>
                                    <span class="t1" v-if="item.taskResult==0">
                                  {{"待审核"}}
                                  </span>
                                    <span class="t1" v-if="item.taskResult==1">
                                    <i :class="'el-icon-circle-check'"></i>
                                    {{"已通过"}}
                                  </span>
                                    <span class="t1" v-if="item.taskResult==2">
                                ...
                                <div class="word_line">
                                  ...
                                  <span class="t1">
                                  <i><img src="../../../common/images/examine/icon_time.png" alt=""></i>
                                  {{item.taskTime}}
                                </span>
                                </div>
                                <div class="word_line">
...
import {getCustomerexamine, customerDetailByid, pagingQuery } from "@/api/customer/index";
...
export default {
  ...
  filters:{
    timer(val){
      if(val && typeof val == 'string' && val.substring!==undefined){
  ...
  created() {
    ...
    if(this.$route.query.id){
            ...
            this.auditId = this.$route.query.id
              ...
              pagingQuery(form).then(res => {
                let auditData = res.data.list[0];
                this.auditDatas = res.data.list[0];
              this.auditDetails = res.data.list[0].auditTasks
              this.auditDetail = res.data.list[0].auditTasks
              this.indexa = auditData.auditTasks.length - 1;
              console.log(auditData.auditTasks.length)
              res.data = res.data.list[0].formData
              let data = res.data
              this.form = { ...data }
  ...
  methods: {
    tabIndex(index) {
      ...
      this.indexa = index;
      console.log(this.indexa-1)
      console.log(this.auditDetail[index])
      if (index == 0) {
