  ...
  <div class="margin">
    ...
    <div class="mcard">
      ...
      <el-table   :data="tableData"  border style="width: 100%" stripe>
        ...
        <el-table-column  prop="auditTasks"  label="审核说明" align="center"  width="100">
          <template slot-scope="scope">
            <!-- <div class="txt_left">操作: {{scope.row.remark}}</div>
            <div class="txt_left">审核人：</div> -->
            <div v-for="(detail,index) in scope.row.auditTasks" class="txt_left">
              <!-- {{index+1}}.{{detail.userName}}{{detail.roleName?'('+detail.roleName+')':''}} -->
              <!-- <span v-if="detail.taskResult==0">待审核
                <el-button type="success" size="small" @click="editTaskUserView(detail.taskId)" style="padding: 3px 3px;">重新指派</el-button>
              </span> -->
              <span>{{detail.taskComment}}</span>
              <!-- <span v-if="detail.taskResult==1">通过</span>
              <span v-if="detail.taskResult==2">未通过</span> -->
            </div>
            ...
            <!-- <div class="txt_left">{{scope.row.remark}}</div> -->
          </template>
        </el-table-column>
        <el-table-column  prop="remark"  label="审核理由" align="center"  width="100">
        </el-table-column>
        <el-table-column  prop="formData.enterBussNo"  label="统一社会信用代码" align="center"></el-table-column>
        <el-table-column  prop="formData.enterprieName"  label="企业名称" align="center"></el-table-column>
        <el-table-column  prop="formData.enterprieNature"  label="企业类型" align="center">
        </el-table-column>
        <el-table-column  prop="formData.contactMobile"  label="联系人电话" align="center"></el-table-column>
        <el-table-column  prop="formData.department"  label="部门" align="center" ></el-table-column>
        <el-table-column  prop="formData.mobile"  label="手机号码" align="center"></el-table-column>
        <el-table-column  prop="formData.telephone"  label="办公电话" align="center"></el-table-column>
        <el-table-column  prop="formData.enterprieAddress"  label="地址" align="center"></el-table-column>
        <el-table-column  prop="formData.mailbox"  label="联系人邮箱" align="center"></el-table-column>
        <el-table-column  prop="formData.userName"  label="账号" align="center"></el-table-column>
        ...
        <el-table-column label="操作" align="center"  fixed="right"  width="150">
          <template slot-scope="scope">
              <div class="small-Idiv" @click="detail(scope.row.auditId,1)" v-permit="'enterprise:audit'">
                <div class="i-div">
                  <img src="../../../common/images/icon/查看详情.png" alt="" class="operation_img">
                </div>
                <span>详情</span>
              </div>
              <!-- <div v-if="(scope.row.auditStatus!=0)" class="small-Idiv"  @click="deleteAuditView(scope.row.auditId)" v-permit="'enterprise:remove'">
              ...
              <div class="small-Idiv" @click="_throughSupplieraudit(scope.row)"  v-permit="'enterprise:check'"
              ...
              <!-- <div class="small-Idiv" @click="distribution(scope.row)"  v-permit="'enterprise:allot'"
...
<script>
import { getCustomerexamine,deleteCustomer,customerToExamine, auditDelete , customerAudit, pagingQuery,customerToExamineCommpany} from '@/api/customer/index'
  ...
  methods: {
    ...
    examine(){
      let obj ={
        ...
        taskId: this.taskId,
        verifyRemark: this.textarea
      }

      console.log(obj)
      customerToExamineCommpany(obj).then(res =>{
    ...
    detail(id,state) {
      this.$router.push({ name: "enterpriseExamineDetail", query: { id: id ,state:state} });
    },
    editor(id,state) {
      console.log(id);
      ...
      _getCustomerexamine(slt){
        ...
        let obj = {}
        obj.pageIndex = this.form.current_page
        obj.pageSize = this.form.page_size
        obj.keyword = this.form.keywords
        obj.auditStatus = this.form.auditStatus
        console.log(obj)
        pagingQuery(obj).then(res => {
      ...
      _throughSupplieraudit(val,type){
        ...
        val.auditTasks.forEach((item) => {
          if(userData.sysuserId === item.userId){
