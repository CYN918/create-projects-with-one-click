        ...
        <div class="goods-fitter">
          <div class="search-wrapper">
            ...
            <el-row>
              ...
              <el-col :span="5">
                <el-form-item label="销售管理员:" prop="salesRepresentative">
...
import { customerDetailByid, getindustryCategory} from "@/api/customer/index";
export default {
  data() {
    return {
      currentIndex: 0,
  ...
  created() {
    ...
    if(this.$route.query.id){
            ...
            customerDetailByid(this.customerId).then(res => {
              this._getindustryCategory(res.data.industryCategory)
  ...
  methods: {
    //获取行业大类
    async _getindustryCategory(val) {    
      const { data } = await getindustryCategory(2);
      let parentName = null;
      data.forEach((value,index) => {
        if (value.id == val) {
          parentName = value.industryName;
        }
      });
      this.form.industryCategory =  parentName//行业大类
    },
    tabIndex (index) {
