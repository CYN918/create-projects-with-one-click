<template>
  <div class="editOrder">
                    ...
                    <el-row>
                      <el-col :span="7">
                        <el-form-item label="调拨单号：" prop="customerName">
                          <el-input
                            v-model="editDatas.transferBillNo"
                            placeholder="保存后自动生成"
                            readonly
                          ></el-input>
                        </el-form-item>
                    ...
                    <el-row>
                      ...
                      <el-col :span="7" :offset="1">
                        <el-form-item label="调入仓库：" prop="invoiceType">
                          <el-select
                            v-model="editDatas.toWarehouseId"
                            placeholder="必填项"
                            style="width:100%;"
                          >
                            <el-option
                      ...
                      <el-col :span="7">
                        <el-form-item label="调出仓库：" prop="invoiceType">
                          <el-select
                            v-model="editDatas.fromWarehouseId"
                            placeholder="必填项"
                            style="width:100%;"
                          >
                            <el-option
    ...
    <!-- 编辑客户询价添加商品弹窗 -->
    <el-dialog width="68%" :visible.sync="innerVisible1" :modal="true" :append-to-body="false">
      <!-- 新增客户询价添加商品选择品牌弹窗 -->
        ...
        if(val.length>0){
          for(let i=0;i<val.length;i++){
            if(val[i].goodsCount>val[i].balanceCount){
              throttleTipPop(this, "error", "调拨数量不能大于实际库存数量", 2000);
  ...
  methods: {
    ...
    addSelectionChange(row) {
      ...
      for (let i = 0; i < this.selectData.length; i++) {
        ...
        this.selectData[i].totalPrice = this.selectData[i].balanceTotalPrice;
      }
      arr.forEach(v => {
        v["goodsCount"] = "0";
        v["reason"] = "";
    ...
    cancelGoods() {
      if (this.detailId) {
        let obj = {};
        obj.transferBillId = this.sendId;
        obj.detailId = this.detailId;
        deleteStockGoods(obj).then(res => {
          if (res.code === 200) {
            this.editDatas.details.splice(this.deIndex1, 1);
    ...
    handleSelectionChange(val) {
      if (val.length > 1) {
        this.$nextTick(function() {
          ...
          this.$refs.elTables.clearSelection();
          this.$refs.elTables.toggleRowSelection(newArr);
        });
      }

      if (val[0]) {
        if (val[0] && val[0].detailId) {
          this.deIndex = "";
          this.detailId = val[0].detailId;
    ...
    editOrder() {
      this.editDatas.goods = this.editDatas.details;
      updateStockDetail(this.editDatas).then(res => {
        if (res.code === 200) {
          throttleTipPop(this, "success", "修改调拨单成功", 2000);
