<template>
  <div id="pendingContract3">
    <div class="margin0">
      <div class="mbottom">
        <div class="goods-fitter">
          <el-form label-width="100px" size="small">
            <div class="search-wrapper">
              <h4 class="title">库存盘点</h4>
                ...
                <el-row>
                  ...
                  <el-col :span="5">
                    <el-form-item label="仓库：" prop="invoiceType">
                      <el-col :span="21">
                        <el-select
                          v-model="tableDatas.warehouseId"
                          placeholder="必填项"
                          style="width:100%;"
                        >
                          <el-option
                  ...
                  <el-col :span="5">
                    <el-form-item label="单号：">
                      <el-col :span="21">
                        <el-input v-model="tableDatas.billNo" placeholder="请输入盘点单号"></el-input>
                      </el-col>
                    </el-form-item>
                  </el-col>
                  <el-col :span="2">
      ...
      <div class="mcard" style="border:1px solid #d7e0f1">
        ...
        >
          ...
          <el-table-column label="盘点单号" prop="name" align="center" width="230">
            <template slot-scope="scope">{{scope.row.billNo}}</template>
          </el-table-column>
          <el-table-column label="盘点日期" prop="name" align="center" width="200">
            <template slot-scope="scope">{{scope.row.bizTime}}</template>
          </el-table-column>
          <el-table-column label="盘点仓库" prop="name" align="center">
            <template slot-scope="scope">{{scope.row.warehouseName}}</template>
          </el-table-column>
          <el-table-column label="盘点人员" prop="name" align="center">
            <template slot-scope="scope">{{scope.row.bizUserName}}</template>
          </el-table-column>
            ...
            <template slot-scope="scope">
              <div class="line-div">
                <div
                  ...
                  v-permit="'inventory:save_push'"
                >
                  <div class="i-div">
                    <img src="@/common/images/icon/提交调拨单.png" alt class="operation_img" />
                  </div>
                  <span style="word-break: keep-all;">提交盘点单</span>
                </div>
                <div class="small-Idiv" @click="detail(scope.row)" v-permit="'inventory:detail'">
                  <div class="i-div">
                ...
                <div class="small-Idiv" @click="editOrder(scope.row)" v-permit="'inventory:update'">
                  <div class="i-div">
                ...
                <div
                  ...
                  v-permit="'inventory:audit'"
                >
                  <div class="i-div">
                    <img src="@/common/images/icon/提交调拨单.png" alt class="operation_img" />
                  </div>
                  <span style="word-break: keep-all;">审核盘点单</span>
                </div>
              </div>
            </template>
          </el-table-column>
      ...
      <div class="margin0" style="margin-top:20px">
        ...
        <!-- 新增销售订单 -->
        <el-dialog :visible.sync="addVisible" width="75%" :before-close="addClose">
          <div class="goods-fitter" style="margin-bottom:20px">
            <el-form label-width="100px" size="small">
              <div class="search-wrapper">
                <h4 class="title">新增盘点单</h4>
              </div>
            </el-form>
          </div>
                  ...
                  <el-form label-width="100px" size="small">
                    ...
                    >
                      ...
                      <el-table-column label="图样" align="center" width="110">
                        <template slot-scope="scope">
                          <div class="image" v-if="scope.row.goodsImgUrl">
                            <img :src="scope.row.goodsImgUrl" width="95" height="95" alt />
                          </div>
                          <div v-if="!scope.row.goodsImgUrl">暂无图片</div>
                        </template>
                      </el-table-column>
        ...
        <!-- 提交盘点单 -->
        <el-dialog width="55%" :visible.sync="auditVisible" :before-close="auditClosing">
          <div class="mbottom">
            <div class="goods-fitter">
              <el-form label-width="100px" size="small">
                <div class="search-wrapper">
                  <h4 class="title">提交盘点单</h4>
                </div>
              </el-form>
            </div>
          </div>
...
export default {
      ...
      handler(val) {
        if (val.length > 0) {
          for (let i = 0; i < val.length; i++) {
            val[i].diffPrice =
              val[i].balanceTotalPrice - val[i].goodsTotalPrice;
            val[i].diffCount = val[i].balanceCount - val[i].goodsCount;
          }
        }
      },
      deep: true
    ...
    total1() {
      ...
      if (_this.goodsData.length > 0) {
        for (var i = 0; i < _this.goodsData.length; i++) {
          if (!isNaN(_this.goodsData[i].goodsTotalPrice)) {
            totals += parseFloat(_this.goodsData[i].goodsTotalPrice);
          } else {
            totals = 0;
          }
        }
  ...
  methods: {
    ...
    //获取盘点分页
    _getInventoryList() {
      getInventoryList(this.tableDatas).then(res => {
        if (res.data) {
    ...
    //调拨单审核弹框
    submitAudit(row) {
      this.sendId = row.billId;
      getStorage().then(res => {
        this.fromData = res.data;
        this.toData = res.data;
      });
      getInventoryDetail(row.billId).then(res => {
        this.orderDatas = res.data;
      });
      InventoryPersonData(row.billId).then(res => {
        if (res.code === 200) {
    ...
    AhandleSelectionChange(row) {
      ...
      this.showId = row.billId;
      let obj = {};
      obj.pageIndex = this.goodsList.pageIndex;
      obj.pageSize = this.goodsList.pageSize;
      obj.billId = row.billId;
      getInventoryGoodsList(obj).then(res => {
    ...
    _addOrder() {
      ...
      addInventory(this.form).then(res => {
        if (res.code === 200) {
          this.$message({ message: "新添盘点单成功", type: "success" });
          this.addVisible = false;
          this.ruleForm = {};
          this.options1 = [];
          this._getInventoryList();
    ...
    //提交挑拨单
    submitOrder(row) {
      initInventory(row.billId).then(res => {
        if (res.code === 200) {
    ...
    pushAuditSave() {
      let obj = {};
      obj.orderId = this.pushData.id;
      obj.reason = this.form.reason;
      obj.limitTime = this.form.limitTime;
      pushInventoryAudit(obj).then(res => {
        if (res.code === 200) {
          throttleTipPop(this, "success", "提交盘点单成功", 2000);
          this.auditVisible = false;
          this._getInventoryList();
        }
      });
