  ...
  <div class="margin0">
    <div class="mcard" style="border:1px solid #d7e0f1">
      <el-table :data="inPage" border @selection-change="handleSelectionChange" stripe>
        ...
        <el-table-column label="商品信息" prop="name" align="center" width="240">
            <template slot-scope="scope">
        ...
        <el-table-column label="规格尺寸" prop="name" align="center" width="200">
                <template slot-scope="scope">
                    {{scope.row.specSize}}
                </template>
        </el-table-column>
        <el-table-column label="单位" prop="name" align="center" width="70">
                <template slot-scope="scope">
                      {{scope.row.specSize}}
                </template>
        </el-table-column>
        <el-table-column label="盘点前库存数量" prop="name" align="center">
              <template slot-scope="scope">
                   {{scope.row.beforeGoodsCount}}
              </template>
        </el-table-column>
         <el-table-column label="盘点后库存数量" prop="name" align="center">
              <template slot-scope="scope">
                    {{scope.row.goodsCount}}
              </template>
        </el-table-column>
        <el-table-column label="单价" prop="name" align="center" width="100">
              <template slot-scope="scope">
        ...
        <el-table-column label="合计" prop="name" align="center">
         ...
         <el-table-column label="盘点后库存余额" prop="name" align="center">
              <template slot-scope="scope">
                    {{scope.row.goodsTotalPrice}}
              </template>
        </el-table-column>
        <el-table-column label="差数" prop="name" align="center" width="70">
              <template slot-scope="scope">
                     {{scope.row.diffCount}}
              </template>
        </el-table-column>
        <el-table-column label="差额" prop="name" align="center" width="100">
              <template slot-scope="scope">
                    {{scope.row.diffPrice}}
              </template>
        </el-table-column>
        <el-table-column label="差额原因" prop="name" align="center" width="175">
              <template slot-scope="scope">
                     {{scope.row.reason}}
              </template>
        </el-table-column>
          ...
          <div class="total" style="margin-left:56.5%;margin-top:20px">
      ...
      handler(val){
        if(val&&val.length>0){
          for(let i =0;i<val.length;i++){
            val[i].totalPrice = val[i].goodsCount * val[i].goodsPrice;
          }
        }
      }
    }
