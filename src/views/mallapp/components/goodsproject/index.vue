  ...
  <div class="margin">
    ...
    <div class="createGoods">
        ...
        <div style="text-align: center;margin-bottom: 50px;">
          ...
          <el-button type="primary" @click="sureCreate">确定创建</el-button>
        </div>
    </div>
      ...
      addGoodsProject(obj).then(res => {
        if(res.code == 200){
          ...
          newSpecialGoods(arr).then(res => {
            if(res.code == 200){
              batchUpdateSpecialGoods(res.data).then(res => {
                if(res.code == 200){
                  this.$message({
                    message: res.message,
                    type: 'success'
    ...
    handlepushList(row){
      ...
      row.forEach((item) => {
        ...
        let obj = {
          ...
          sortOrder: item.sortOrder,
          thumbnailImgUrl: item.skuInfo[0].thumbnailImgUrl,
          goodsPrice: item.skuInfo[0].priceInfo.goodsPrice,
          costPrice: item.skuInfo[0].priceInfo.costPrice,
        }
        // item.skuInfo.forEach((val) => {
        //   let obj1 = {
        //     thumbnailImgUrl: val.thumbnailImgUrl,
        //     goodsPrice: val.priceInfo.goodsPrice,
        //     costPrice: val.priceInfo.costPrice
        //   }
        //   let newObj = {};
        //   Object.assign(newObj,obj,obj1);
          this.myArray.push(obj)
          this.heavy(this.myArray)
          return this.myArray
        // })
      })
      // this.myArray = row;
