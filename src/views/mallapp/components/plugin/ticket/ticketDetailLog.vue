...
import { getTicketLogList } from '@/api/mallapp/plugin/index'
...
export default {
  ...
  methods: {
    getTableData () {
      ...
      return getTicketLogList(this.tableSubmitData)
    },
    routerToCodeDetail (item) {
      this.$router.push({ path: '/mplugin/score_log_detail', query: { id: item.log_id } })
    }
  }
}
</script>
