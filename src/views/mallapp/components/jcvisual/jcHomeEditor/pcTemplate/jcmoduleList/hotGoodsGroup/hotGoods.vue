				...
				<div class="groupActivityWrap" v-show="isshow">
						...
						<div class="preemptivePurchase" @mouseenter="isShowBtn=true" @mouseleave="isShowBtn=false">
								...
								<div class="newActivity">
										<!--表格内容-->
										<ul style="display: flex;flex-direction: column;background-color: #fff;height: 512px;padding: 20px">
												<li v-for="(item,index) in (hotGoodsList||[]).slice(0,4)"
														style="border-bottom: 1px solid #e6e6e6;">
														<div
																style="display: flex;justify-content: center;align-items: center;height:118px;background-color: #fff;margin-bottom: 1px">
																<img :src="item.gradeImgURL" alt="" style="width: 30px;height: 38px;margin-right: 17px">
																<img :src="item.skuImg" alt=""
																		 style="width: 90px;height: 90px;margin-right: 14px">
																<div
																		style="display: flex;flex-direction: column;justify-content: center;width: 404px;height: 86px;overflow: hidden;font-size: 16px;color: #333;">
																		<span style="color: #0b0d0e">{{item.goodsName}}</span>
																		<div
																				style="display: flex;justify-content: flex-start;align-items: center;;margin-top: 10px">
																				<span
																						style="color: #e93632;font-size: 18px;font-weight: 700;margin-right: 10px">¥{{item.minimumPrice}}</span>
																				<div
																						style="background-color: #000000;color:#fff;width:80px;height: 20px ;font-size: 12px;line-height: 20px;text-align: center;">
																						企业专享受价
																				</div>
																		</div>
																</div>
						...
						<!--热爆的弹框-->
						<el-dialog :title="`${advertisingSlogan}编辑`" :visible.sync="hotGoodsDialogVisible" width="965px">
        ...
        methods: {
            ...
            getActivity() {
                ...
                if (content && content.length > 0) {
                    ...
                    this.hotGoodsList = contentArr.activityGoods
                    console.log(99, this.hotGoodsList)
                    this.hotGoodsList = this.hotGoodsList.map(v => {
                        this.$set(v, 'gradeImgURL', '')
                        return v
                    })
                    this.hotGoodsList.forEach((item, index) => {
                        if (index == 0) {
                            item.gradeImgURL = require('@/common/images/mlm/hotGoods/1.png')
                        } else if (index == 1) {
                            item.gradeImgURL = require('@/common/images/mlm/hotGoods/2.png')
                        } else if (index == 2) {
                            item.gradeImgURL = require('@/common/images/mlm/hotGoods/3.png')
                        } else {
                            item.gradeImgURL = require('@/common/images/mlm/hotGoods/4.png')
                        }
                    })
                    this.form = {
